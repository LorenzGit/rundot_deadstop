...
async function initializeRenderer(host: HTMLElement, preference: RendererPreference): Promise<Application> {
    ...
    try {
        await app.init({
            ...
            autoDensity: true,
            background: PALETTES.ledger.css.page,
            // Ink strokes are vector art; smooth edges are the whole point.
            antialias: true,
        });
        return app;
    } catch (error) {
...
export async function createPixiApp(host: HTMLElement): Promise<Application> {
    ...
    app.canvas.setAttribute("aria-label", "DEADSTOP inked firefight page");
