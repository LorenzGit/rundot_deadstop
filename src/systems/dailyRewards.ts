...
export function dailyRewardsView(): DailyRewardsView {
    ...
    return {
        ...
        nextLabel: claimedToday
            ? `NEXT PAGE IN ${formatDailyCountdown(msUntilNextLocalMidnight(serverNow()))}`
            : "TODAY'S INK IS READY",
    };
}
...
export async function claimDailyReward(): Promise<{ ok: boolean; message: string }> {
    ...
    if (before.dailyRewards.claimIds.includes(claimId)) {
        return { ok: false, message: "TODAY'S INK ALREADY CLAIMED" };
    }
    ...
    claimInFlight = true;
    const applied = saveSystem.applyDailyReward({ day: gate.day, ink: reward.ink });
    if (!applied.ok) {
        claimInFlight = false;
        return { ok: false, message: "TODAY'S INK ALREADY CLAIMED" };
    }
    ...
    if (!saved) {
        // Delta revert, not a snapshot restore: ink earned elsewhere while the
        // flush was in flight must survive the rollback.
    ...
    claimInFlight = false;
    return { ok: true, message: `+${reward.ink} INK` };
}
