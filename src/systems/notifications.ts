 ...
 * the daily page of ink. There is deliberately no streak-nag, no "we miss you",
...
const DAILY_REMINDER_ID = "deadstop-daily-page";
 ...
 * free ink is how a game gets its notifications turned off for good.
 */
...
export async function syncDailyReminder(): Promise<boolean> {
    ...
    if (!permissionChecked) await refreshNotificationPermission();
    ...
    const scheduled = await scheduleLocalNotification({
        ...
        title: "A FRESH PAGE",
        body: "Your daily ink is waiting. Time only moves when you do.",
        delaySeconds,
        collapseKey: DAILY_REMINDER_ID,
    });
    recordAnalytics("daily_reminder_scheduled", { scheduled, delaySeconds });
    return scheduled;
}
